...
const DoctorSidebar = () => {
  ...
  const navItems = [
    { icon: <Home size={20} />, label: "Profile" },
    { icon: <Users size={20} />, label: "Patient-Panel" },
    { icon: <FileText size={20} />, label: "Pending-Appointments" },
  ];
              ...
              {navItems.map((item, index) => (
                <li key={index}>
                  ...
                  >
                    <span
                      onClick={() =>
                        navigate(`/doctor/${item.label.toLowerCase()}`)
                      }
